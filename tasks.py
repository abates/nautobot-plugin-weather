...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NAUTOBOT_WEATHER_xxx
namespace = Collection("nautobot_weather")
namespace.configure(
    {
        "nautobot_weather": {
            "nautobot_ver": "1.6",
            "project_name": "nautobot_weather",
            "python_ver": "3.8",
            "local": False,
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.nautobot_weather.compose_http_timeout,
        "NAUTOBOT_VER": context.nautobot_weather.nautobot_ver,
        "PYTHON_VER": context.nautobot_weather.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.nautobot_weather.project_name}",
        f'--project-directory "{context.nautobot_weather.compose_dir}"',
    ]

    for compose_file in context.nautobot_weather.compose_files:
        compose_file_path = os.path.join(context.nautobot_weather.compose_dir, compose_file)
...
def run_command(context, command, **kwargs):
    ...
    if is_truthy(context.nautobot_weather.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_weather.python_ver}...")
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations nautobot_weather"
...
def dbshell(context, query="", input="", output=""):
    ...
    if "docker-compose.mysql.yml" in context.nautobot_weather.compose_files:
    ...
    elif "docker-compose.postgres.yml" in context.nautobot_weather.compose_files:
        command += [
            "--",
            service,
            "psql",
...
def pylint(context):
    ...
    command = 'pylint --init-hook "import nautobot; nautobot.setup()" --rcfile pyproject.toml nautobot_weather'
    run_command(context, command)
...
def unittest(context, keepdb=False, label="nautobot_weather", failfast=False, buffer=True, pattern=""):
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'nautobot_weather/*' --omit *migrations*"
...
def tests(context, failfast=False):
    ...
    if not is_truthy(context.nautobot_weather.local):
